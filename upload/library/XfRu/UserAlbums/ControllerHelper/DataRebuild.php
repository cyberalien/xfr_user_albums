 ...
 *
 */
class XfRu_UserAlbums_ControllerHelper_DataRebuild extends XenForo_ControllerHelper_Abstract
{
	public function rebuildData(array $input, $redirect, $submitUrl, $doRebuild = false)
	{
		$caches = $input['caches'];
		...
		if ($doRebuild && $caches)
		{
			...
			$rebuilt = XfRu_UserAlbums_CacheRebuilder_Abstract::getCacheRebuilder($cache)->rebuild($position, $options, $detailedMessage);
			if (is_int($rebuilt))
			{
		...
		$nextBuilder = XfRu_UserAlbums_CacheRebuilder_Abstract::getCacheRebuilder($nextCache);
